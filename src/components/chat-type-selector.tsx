...
export default function ChatTypeSelector({
  ...
  const selectedChatTypeList = selectedChatTypes ? Object.entries(selectedChatTypes).filter(([_, selected]) => selected).map(([chatType]) => chatType) : [];
      ...
      <PopoverTrigger asChild>
        ...
        >
          {selectedChatTypeList.length > 0
            ? (chatTypeFilteringMode === "whitelist" ? "" : "!") + selectedChatTypeList[0] + (selectedChatTypeList.length > 1 ? ", ..." : "")
            : "Select chat type..."}
        ...
        <Command>
          <CommandGroup heading="Settings">
            <CommandItem className="pl-6" onSelect={() => togglePlayerFilteringMode()}>
              ...
              {chatTypeFilteringMode === "whitelist" ? "Whitelist" : "Blacklist"}
            </CommandItem>
          </CommandGroup>
          <CommandSeparator />
          <CommandList>
            ...
            <CommandGroup heading="Players">
              {(Object.keys(ChatTypeFilterDefault) as ChatType[]).sort().map((chatType, i) => (
                <CommandItem
                  key={i}
                  value={chatType}
                  onSelect={(currentValue) => {
                    const newSelectedChatTypes: ChatTypeFilter = selectedChatTypes ? {
                      ...selectedChatTypes,
                    } : {...ChatTypeFilterDefault};
                    if(Object.keys(newSelectedChatTypes).includes(currentValue)) {
                      const chatType = currentValue as ChatType;
                      newSelectedChatTypes[chatType] = !newSelectedChatTypes[chatType];
                      if(Object.values(newSelectedChatTypes).some((v) => v)) {
